...
let totalOptions = { width: 200, height: 200, redFrom: 800, redTo: 1000, yellowFrom: 600, yellowTo: 800, minorTicks: 5, max: 1000, animation: { duration: 1500, easing: 'out' } };
let countOptions = { width: 200, height: 200, redFrom: 80, redTo: 100, yellowFrom: 60, yellowTo: 80, minorTicks: 5, max: 100, animation: { duration: 1500, easing: 'out' } };
let tableOptions = { 
    width: '100%', 
    showRowNumber: true, 
    cssClassNames: {
...
function drawCharts() {
    totalData = google.visualization.arrayToDataTable([['Label', 'Value'], ['Total', 0]]);
    totalGauge = new google.visualization.Gauge(document.getElementById('total-gauge'));
    totalGauge.draw(totalData, totalOptions);

    countData = google.visualization.arrayToDataTable([['Label', 'Value'], ['Count', 0]]);
...
function animateStatus(sum, count) {
    ...
    const sumStep = sum / 50, countStep = count / 50;
    const interval = setInterval(() => {
        currentSum += sumStep;
